...
2. **选择视觉风格并确认参考图。** 向用户展示 [内置风格](#风格选择)，让用户选择其中一种、提供自定义风格 Prompt，或复用已经确认的风格参考图。使用选定 Prompt 生成风格参考图并经用户确认，再根据全片分镜列出人物，为主要人物和重要配角准备独立人物参考图。
...
选定内置风格后，完整读取对应文件，将其中的“风格 Prompt”作为生成风格参考图的基础描述。可以根据画面主题补充具体主体和场景，但不得改写其中的媒介、材质、线条、配色、光影和整体气质，也不得把某个具体作品、人物、场景或画幅反向固化进风格文件。

风格 Prompt 用于稳定生成同类视觉语言，但文字提示不能保证每次输出完全一致。真正用于全片一致性控制的是用户确认的风格参考图：先生成候选参考图，经用户确认后再用于人物参考图和全部视频片段；未确认前不得批量生成视频。
