...
description: 制作哲学、心理、历史、经济、金融与科技等主题的 AI 动画解说视频。适用于 Codex 需要研究和组织选题、编写旁白、将 1–5 分钟视频拆成均衡的 Seedance 片段、从随 skill 分发的音色与图像参考库选择资产、设计参考图或多镜头视频提示词、保持旁白音色一致、制作带字封面，或规划基于 CLI 的多模态生成与拼接流程。当前正式维护的执行路径使用 LibTV CLI；查阅最新官方文档后，也可将该生产方法适配到 Higgsfield、即梦及其他同类多模态 CLI。
---
...
1. **写作并拆分完整讲稿。** 先向用户确认主题、受众和成片时长，再完成全文、核查事实并按语义拆成片段。执行前完整读取 [旁白讲稿写作指南](references/narration-script-guide.md)；将完整讲稿和拆分结果交给用户审阅，获得确认后再进入视觉设计。
2. **选择视觉风格并确认参考图。** 读取[内置参考资产库](references/reference-asset-library.md)，明确询问用户是选择随 skill 分发的内置图像参考、只选择一种[内置风格](#视觉风格与参考资产)、提供自定义风格、复用其他已确认参考图，还是在内置风格之外新建一个 style；不得代替用户决定。读取选定 style 的图像指导，为当前内容重新编写图像 Prompt；视觉风格、风格参考图、主要人物和重要配角参考图均经用户确认后，才能正式生成视频。
3. **为全部片段编写并保存视频 Prompt。** 旁白定稿后，完整读取所选 style 的视频指导、[视频提示词写作指南](references/video-prompt-guide.md)和 [Seedance 2.0 官方提示词指南](references/official-seedance-2.0-prompt-guide.md)，逐段设计素材职责、镜头、动作和切换。把旁白中的思想优先改写成发生在可识别场景里的具体人物、物体、行动和结果，不用抽象概念画面承担主要表达。生成后按照视频提示词指南保存为当前作品的 Markdown 文档，不得只保留在对话中。
...
| 选择或使用随 skill 分发的音色与图像参考 | [内置参考资产库](references/reference-asset-library.md) |
...
结合 style 的图像指导生成候选参考图并经用户确认，再批量生成视频；不得在片段间静默更换参考。图像只负责画风时，Prompt 固定只写“参考图 1 的画风。”，不复述风格描述，也不默认复制其主体、构图、文字或情节。人物参考、群演画风和素材职责的详细规则统一由[视频提示词写作指南](references/video-prompt-guide.md)维护。

随 skill 分发的正式音色与图像参考统一登记在[内置参考资产库](references/reference-asset-library.md)，其 `assets/reference-library/` 文件是公开库的唯一正式媒体源。上传到宿主项目或平台画布的节点只是运行时副本，不得反向取代或静默覆盖 skill 资产。

开始新视频时先读取内置与宿主项目补充的图像参考，让用户选择复用或新生成；不得把图像选择推断为音色选择。正式生成视频前读取内置音色清单，明确询问用户使用已有音色还是新建音色，不设置静默默认值。宿主项目可以额外提供私有参考，但不得扫描历史作品音频冒充库资产。新建音色仍随片段 1 一起生成，默认只服务当前作品；只有用户明确确认纳入公开库后，才按音色指南写入 skill，私有资产则留在宿主项目。

为已确认图像参考资产记录稳定标识、类型、用途、描述、完整生成 Prompt、来源平台及工作区／画布／节点、本地路径和确认状态。修改时创建新版本并重新确认，不覆盖或静默替换；在 LibTV 中，同画布直接连接已有节点，跨画布则先用 CLI 下载再上传到目标画布，不直接跨画布连接节点。本期音色锚点只服务当前视频，按音色指南单独记录与管理。
...
- 主要人物或重要配角缺少已确认参考图，或参考素材职责与实际连接不一致。
