...
- 不用视频片段直接代替音频参考。通过 LibTV CLI 下载已确认的片段 1，使用 macOS `afconvert` 输出 48kHz、双声道、16-bit PCM WAV，再通过 LibTV CLI 上传为独立音频节点；后续只连接纯音频。该 WAV 格式已验证可通过 Seedance 2.0 合规检测，M4A 曾被拒绝。视频参考成本显著更高，不得仅为音色而连接整段视频。
