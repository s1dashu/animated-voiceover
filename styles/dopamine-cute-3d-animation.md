...
- 生成本风格的图像参考时，优先使用 Nano Banana Pro；已确认它更容易同时实现 Q 弹角色、鲜明多巴胺配色、丰富满画幅内容与清楚的构图层级。
...
3. 生成人物参考图时锁定头身比例、体块轮廓、五官位置、服装色块、材质和标志性配件；不同人物应在轮廓与形状语言上明显可区分。
4. 生成风格参考图时优先展示多个角色、丰富环境、远近层次和高明度色彩关系，以检验“内容很多但结构清晰”是否同时成立。
...
4. 保持高明度色块、圆润材质和清楚轮廓的一致性；不同镜头可以改变场景、主体数量、景别和主色，不能机械复刻参考图的构图。
