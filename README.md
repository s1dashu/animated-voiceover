...
- 从可扩展的风格目录中选择视觉语言，并建立经用户确认的风格参考图。
- 规划人物参考和多镜头导演方案。
...
每种内置风格都有一份独立、可直接复用的精准 Prompt。先选择风格并生成候选参考图，经用户确认后，再用参考图约束整支视频；也可以提供自定义风格 Prompt 或自己的参考图。

| 风格 | 视觉特征 | Prompt |
| --- | --- | --- |
| 电影感 3D 动画 | 哑光手绘材质、低饱和蓝灰、体积光与胶片感 | [查看风格 Prompt](./styles/cinematic-3d-animation.md) |
| 黏土定格动画 | 手工黏土偶、微缩布景、指纹和塑形痕迹 | [查看风格 Prompt](./styles/clay-stop-motion.md) |
| 笨拙感手绘动画 | 不规则线条、干粉彩与彩铅、粗纸颗粒 | [查看风格 Prompt](./styles/clumsy-hand-drawn-animation.md) |

## 核心流程

1. 编写并确认完整旁白讲稿。
2. 选择内置或自定义视觉风格，生成并确认风格参考图和人物参考图。
3. 为全部片段编写视频提示词。
